    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
                ...
                {categories.length > 0 && (
                    <div ref={dropRef} className="relative sm:w-80 shrink-0">
                        {/* Trigger */}
                        <button
                            onClick={() => setDropdownOpen((o) => !o)}
                            className={`w-full rounded-2xl border px-4 py-3 shadow-sm flex items-center gap-2.5 transition-all duration-200 ${
                                filterCat ? 'bg-indigo-50 border-indigo-300' : 'bg-white border-gray-200'
                            }`}
                        >
                            ...
                            <span className={`flex-1 text-left text-sm font-medium truncate ${filterCat ? 'text-indigo-700' : 'text-gray-500'}`}>
                                {filterCat || 'Semua Kategori'}
                            </span>
                            {filterCat ? (
                                <span
                                    role="button"
                                    onClick={(e) => { e.stopPropagation(); setFilterCat(''); setDropdownOpen(false); }}
                                ...
                                <svg className={`w-4 h-4 shrink-0 transition-transform duration-200 ${dropdownOpen ? 'rotate-180 text-indigo-400' : 'text-gray-300'}`}
                        ...
                        {dropdownOpen && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl border border-gray-100 shadow-xl z-50 overflow-hidden">
                                <div className="max-h-72 overflow-y-auto py-1.5">
                                    <button
                                        onClick={() => { setFilterCat(''); setDropdownOpen(false); }}
                                    ...
                                    {categories.map((c) => (
                                        <button
                                            key={c.id}
                                            onClick={() => { setFilterCat(c.nama); setDropdownOpen(false); }}
                                            className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors ${
